...
# Shared with the "bracket" tool container the same way as
# DECK_EXPORT_DIR above, but read-only in this direction: bracket.py
# writes its self-contained HTML report there, and this app just
# serves back whichever one is newest.
BRACKET_RESULTS_DIR = Path(
    os.environ.get(
        "BRACKET_RESULTS_DIR",
...
def _format_run_timestamp(raw: str):
    """bracket.py logs started_at as datetime.isoformat(). Render that
    as something readable for the history table; fall back to the raw
    string if it's missing or in an unexpected shape."""
    ...
    try:
        ...
        return datetime.fromisoformat(raw).strftime(
            "%b %-d, %Y %-I:%M %p"
        )
...
def _sorted_bracket_reports():

    return sorted(
        BRACKET_RESULTS_DIR.glob("bracket_*.html"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )


def _serve_bracket_report(
    html_path: Path,
    show_history_link: bool,
):
    """Read one of bracket.py's self-contained HTML reports and splice
    in small nav links (back to the dashboard, and — for an archived
    run — over to the history list), since the report is a fully
    self-styled page served as-is rather than poured into base.html."""

    html = html_path.read_text()

    links = (
        '<a href="/" style="position:fixed;top:16px;left:16px;'
        "z-index:999;padding:8px 14px;border-radius:8px;"
        "background:var(--surface);color:var(--text);"
        "border:1px solid var(--line);font-family:'IBM Plex Sans',"
    ...
    if show_history_link:
        ...
        links += (
            '<a href="/bracket/history" style="position:fixed;'
            "top:16px;left:150px;z-index:999;padding:8px 14px;"
...
def latest_bracket(
    ...
    """Show the most recently completed bracket run's standalone HTML
    report, generated by bracket/bracket.py."""

    report_files = _sorted_bracket_reports()
    ...
    return _serve_bracket_report(
        report_files[0],
        show_history_link=len(report_files) > 1,
    )


# Registered before "/bracket/{stem}" below for the same reason
# "/decks/export-all" is registered before "/decks/{deck_id}" —
# Starlette matches routes in registration order, and "history"
# would otherwise be swallowed as a {stem} value.
@app.get("/bracket/history")
def bracket_history(
    request: Request,
):
    """A short list of past bracket runs, newest first, each linking
    to its own archived report."""

    runs = []

    for html_path in _sorted_bracket_reports():
        ...
        if json_path.exists():
            ...
            summary["started_at"] = _format_run_timestamp(
                data.get("started_at")
            )
            summary["lives"] = data.get("lives")
            summary["games_per_match"] = data.get(
                "games_per_match"
            )
            summary["deck_count"] = len(
                data.get("decks") or []
            )
            summary["champion"] = data.get("champion")
            summary["aborted"] = data.get("aborted")

        runs.append(summary)

    return render_template(
        request,
        "bracket_history.html",
        {
            "runs": runs,
        },
    )
...
@app.get("/bracket/{stem}")
def archived_bracket(
    stem: str,
):
    """Show one archived bracket run's report by its file stem (e.g.
    bracket_lives3_20260907_134249), as linked from /bracket/history."""

    if not re.fullmatch(r"bracket_[A-Za-z0-9_]+", stem):
    ...
    html_path = BRACKET_RESULTS_DIR / f"{stem}.html"
